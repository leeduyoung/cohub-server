...
module.exports.getClient = function *(clientId, clientSecret) {
  return db.query('SELECT client_id, client_secret, redirect_uri FROM oauth_clients WHERE client_id = $1 AND client_secret = $2', [clientId, clientSecret])
    .then(function(result) {
...
module.exports.getUser = function *(username, password) {
  return db.query('SELECT user_id FROM users WHERE username = $1 AND password = $2', [username, password])
    .then(function(result) {
      return result.rowCount ? result.rows[0] : false;
    });
};
